...
router.post('/login', async (req, res) => {
    ...
    if (!captchaResponse.data.success) return res.send('Invalid reCAPTCHA. Please try again.');
    ...
    const jwtToken = jwt.sign({ id: user.id, username: user.username, email: user.email }, process.env.JWT_SECRET, { expiresIn: '15m' });
